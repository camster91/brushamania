<template>
	<div class="card-body">
		<form @submit.prevent="submitSchool" method="POST">
			...
			<div class="row">
				<div class="col-md-12">
					<div class="success-message" v-if="successful != null && successful == true">School Updated Successfully</div>
            		<div class="error-message" v-else-if="successful != null && successful == false">{{error_message}}</div>
					<button class="btn btn-primary pull-right save-btn">Update School</button>
				</div>
			</div>
		</form>
	</div>
...
<script>
	...
	export default {
		...
		created() {
			...
			axios.get('/school/edit/' + this.$route.params.school).then((response) => {
                    	...
                    	if (this.school.is_phone_valid == true) {
							axios.patch('/school/' + this.school.id, this.school).then((response) => {
								...
								if(response.data.success) {
									...
									this.$router.push({path:'/schools/' + response.data.url_slug + '/' + this.$route.params.year });
									eventBus.$emit('refreshSchool');
									this.successful = true;
									this.backend_errors = null;
									this.$store.dispatch('getSchools', this.$route.params.year);
								} else {
