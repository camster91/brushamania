<template>
	<div class="card-body">
		<form @submit.prevent="submitParent" method="POST">
			...
			<div class="row">
				<div class="col-md-12">
					<div class="success-message" v-if="successful != null && successful == true">Parent Updated Successfully</div>
            		<div class="error-message" v-else-if="successful != null && successful == false">{{error_message}}</div>
					<button class="btn btn-primary pull-right save-btn">Update Parent</button>
				</div>
			</div>
		</form>
	</div>
                    	...
                    	if (this.parent.is_phone_valid == true) {
							axios.patch('/parent/' + this.parent.id, this.parent).then((response) => {
								...
								if(response.data.success) {
									this.$router.push({path:'/parents/' + response.data.url_slug});
